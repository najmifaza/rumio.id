...
export default function PropertiesClient({ initialProperties }: PropertiesClientProps) {
  ...
  const renderFilterContent = () => (
    <div className="space-y-8">
      {/* Tipe Transaksi */}
      <div>
        <h3 className="text-sm font-bold text-[#0B1528] mb-3">Tipe Transaksi</h3>
          ...
          {["Dijual", "Disewakan", "Keduanya"].map((type) => (
            <label key={type} className="flex items-center gap-3 cursor-pointer group">
              <input
                ...
                onChange={() => toggleArrayItem(transactionType, type, setTransactionType)}
                className="w-4 h-4 rounded border-slate-300 accent-[#0B1528] cursor-pointer"
              />
              <span className="text-sm text-slate-600 group-hover:text-[#0B1528]">{type}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      {/* Jenis Properti */}
      <div>
        <h3 className="text-sm font-bold text-[#0B1528] mb-3">Jenis Properti</h3>
        <div className="space-y-2.5">
          {["Rumah", "Apartemen", "Ruko", "Tanah", "Villa", "Gudang", "Kost"].map((type) => (
            <label key={type} className="flex items-center gap-3 cursor-pointer group">
              <input
                type="checkbox"
                checked={propertyType.includes(type)}
                onChange={() => toggleArrayItem(propertyType, type, setPropertyType)}
                className="w-4 h-4 rounded border-slate-300 accent-[#0B1528] cursor-pointer"
              />
              <span className="text-sm text-slate-600 group-hover:text-[#0B1528]">{type}</span>
            </label>
      ...
      <div className="pt-4 border-t border-slate-100 flex flex-col gap-3">
        <Button className="w-full bg-[#0B1528] hover:bg-[#1a2b4c] text-white h-11 rounded-lg" onClick={() => setIsFilterOpen(false)}>
          Terapkan Filter
        </Button>
        <Button variant="outline" className="w-full h-11 rounded-lg border-amber-200 text-amber-700 hover:bg-amber-50 hover:text-amber-800" onClick={handleReset}>
          Reset Filter
        </Button>
      </div>
    </div>
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-20 font-sans pt-20">
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12 xl:px-0 mt-8">
        <Breadcrumbs className="mb-6" items={[{ label: "Beranda", href: "/" }, { label: "Properti" }]} />

        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-6 mb-8">
          <div>
            <h1 className="text-3xl md:text-[34px] font-bold text-[#0B1528] tracking-tight mb-2">Cari Properti Impian</h1>
            <p className="text-slate-500">Menampilkan {filteredProperties.length} properti yang sesuai dengan kriteria Anda</p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            <Button className="lg:hidden flex items-center justify-center gap-2 bg-white text-[#0B1528] border border-slate-200 hover:bg-slate-50 h-11 px-5" onClick={() => setIsFilterOpen(true)} variant="outline">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="hidden lg:block w-[300px] xl:w-[320px] shrink-0">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sticky top-28 max-h-[85vh] overflow-y-auto no-scrollbar" data-lenis-prevent>
          ...
          {isFilterOpen && (
            <div className="fixed inset-0 z-50 flex lg:hidden">
              <div className="fixed inset-0 bg-black/50 transition-opacity" onClick={() => setIsFilterOpen(false)}></div>
              <div className="relative w-[85%] max-w-sm h-full bg-white ml-auto overflow-y-auto p-6 shadow-xl flex flex-col">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setIsFilterOpen(false)} className="p-2 -mr-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                {renderFilterContent()}
          ...
          <div className="flex-1">
            {filteredProperties.length > 0 ? (
              ...
              <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
                ...
                <h3 className="text-lg font-bold text-[#0B1528] mb-2">Properti tidak ditemukan</h3>
                <p className="text-slate-500 text-sm mb-6 max-w-sm mx-auto">Coba sesuaikan filter Anda dengan mengubah rentang harga, lokasi, atau tipe properti.</p>
                <Button onClick={handleReset} className="bg-amber-100 text-amber-700 hover:bg-amber-200 px-6">
                  Reset Filter
                </Button>
              </div>
            )}
